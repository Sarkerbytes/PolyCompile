# PolyCompile: A Multi-Language Compiler & Virtual Execution Framework

**Course Project Report — Compiler Construction**
...
|----------|-------------------|
| **C** | `int`, `float`, `double`, `char`, `bool`, `void`, `if`, `else`, `while`, `do`, `for`, `switch`, `case`, `default`, `break`, `continue`, `return`, `printf`, `scanf` |
| **C++** | `int`, `float`, `double`, `char`, `bool`, `string`, `void`, `if`, `else`, `while`, `do`, `for`, `switch`, `case`, `default`, `break`, `continue`, `return`, `using`, `namespace`, `std`, `cout`, `cin` |
| **Java** | `int`, `float`, `double`, `char`, `boolean`, `String`, `void`, `if`, `else`, `while`, `do`, `for`, `switch`, `case`, `default`, `break`, `continue`, `return`, `public`, `class`, `static`, `import`, `System`, `out`, `println`, `print`, `Scanner` |
...
```mermaid
flowchart TD
    Src[Source File .c / .cpp / .java] --> Det[Language Detector]
    Det --> Fe{Select Frontend}
    
    Fe -->|C| CFe[C Frontend Lexer & Parser]
    Fe -->|C++| CppFe[C++ Frontend Lexer & Parser]
    Fe -->|Java| JavaFe[Java Frontend Lexer & Parser]
    
    CFe --> AST[Abstract Syntax Tree AST]
    CppFe --> AST
    JavaFe --> AST
    
    AST --> Sem[Semantic Analyzer & Symbol Table]
    Sem --> IRGen[TAC IR Generator]
    IRGen --> Opt[TAC Optimizer - 5 Passes]
    Opt --> CodeGen[Target Assembly Generator]
    Opt --> Exec[Virtual Machine Executor]
    Exec --> Out[Stdout / Console Execution]
```
...
| 6 | `+`, `-` | Left-to-Right |
| 7 | `*`, `/`, `%` | Left-to-Right |
| 8 (Highest)| `!`, unary `-`, unary `+` | Right-to-Left |
...
## Scope Checking
Nested scopes (blocks `{ ... }`) create child symbol tables. Variable lookups traverse upward from current scope to global scope.
...
1. Free Software Foundation. (2020). *Flex - A Fast Lexical Analyzer Generator*. GNU Project.
2. Free Software Foundation. (2021). *Bison - GNU Parser Generator*. GNU Project.

## Websites & Tools
1. GCC, the GNU Compiler Collection: [https://gcc.gnu.org](https://gcc.gnu.org)
2. LLVM Compiler Infrastructure: [https://llvm.org](https://llvm.org)
3. Monaco Editor: [https://microsoft.github.io/monaco-editor/](https://microsoft.github.io/monaco-editor/)
